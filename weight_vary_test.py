...
def next_combination(weights):
	if(weights[2]>1):
		weights[1]=weights[1]+1
	elif(weights[2]==1 and weights[0]<8):
		weights[0]=weights[0]+1
		weights[1]=1
	else:
		return []
	weights[2]=10 - weights[0]-weights[1]
	return w
...
while(len(w)!=0):
	for target in range(len(accepted_users)):
		...
		sim = w[0]*sim_genre + w[1]*sim_year + w[2]*sim_rating
	...
	f_sum = np.sum(result_f_score[iteration])
	if f_sum>f_max:
		f_max=f_sum
		print "new max :", '-'.join(map(str, w))
	iteration = iteration+1
	w = next_combination(w)

f = open('vary_weight_demo.csv', 'w+')
